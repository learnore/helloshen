...
if __name__ == "__main__":
    ...
    print(
        {0: 1, 2: 1, 1: 3}.items()
    )       # dict_items([(0, 1), (2, 1), (1, 3)])

    # 原来时字典！原来时字典！原来时字典！注意这里 sorted 后的数据，返回的是一个数组里面包裹的是元组哦!!是元组哦!!是元组哦!!是元组哦!!是元组哦!!
    print(
        sorted({0: 1, 2: 1, 1: 3}.items(), key=lambda x: (x[1], -x[0]), reverse=True)
    )       # [(1, 3), (0, 1), (2, 1)]

    # 使用两个元素进行升序排序，在第一个元素升序的情况下，第二个元素也升序排序
    print(
        sorted([(6, 4), (6, 8), (6, 7), (7, 1)], key=lambda x: (x[0], x[1]))
    )       # [(6, 4), (6, 7), (6, 8), (7, 1)]
    ...
    # 按照身高、体重、编号的顺序进行排序
    students.sort(key=lambda x: (x[1], x[2], x[0]), reverse=False)      # (x[1], x[2], x[0]) 只是用于排序的优先级
    ...
    """
    sorted_list = sorted(
        ["CA3385", "CZ6678", "CZ0678", "DU7523", "HK4456", "MK0987"],
        key=lambda x: (x[:2], int(x[2:])),
        reverse=False
    )
    print(sorted_list)      # ['CA3385', 'CZ0678', 'CZ6678', 'DU7523', 'HK4456', 'MK0987']
